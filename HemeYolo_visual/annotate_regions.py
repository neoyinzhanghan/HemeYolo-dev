...
def annotate_image(image, label_df, label_type='center-width-height', core_radius=3, conf_thres=None):
    ...
    conf_thres should only be not None if the label_type is 'top-left-bottom-right' and the label_df has 6 columns.
    Raise a user warning but allow the function to continue if conf_thres is declared but the label_type is 'center-width-height'. """
    ...
    for file in tqdm(files):
        ...
        if continuation:
            ...
            if args.label_type == 'center-width-height':
                ...
                # annotate the image
                image = Image.open(os.path.join(args.input_dir, file))
                annotate_image(image, df, label_type=args.label_type, conf_thres=args.conf_thres)
